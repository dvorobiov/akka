...
also :ref:`really easy <custom-router-scala>` to create your own.
...
:class:`ActorRef` for the router is returned. The routees will be created as the router's children.
...
matter how that router would normally routes its messages.
...
.. code-block:: scala
  ...
  router ! Broadcast("Watch out for Davy Jones' locker") // TODO: Use code sample.
...
.. code-block:: scala
  ...
  router ! Broadcast(PoisonPill) // TODO: Use code sample.
...
*not* send it on to its routees. The router will be restarted, which will also result in its
children being restarted. Routees that are children will be restarted, but not routees that were
created externally to the router.


As with the ``PoisonPill`` message, there is a distinction between killing a router, which
indirectly kills its children, and killing routees directly (some of whom may not be children.) To
kill all routees the router should be sent a ``Kill`` message wrapped in a ``Broadcast`` message.
...
Before creating your own actor you should consider whether a normal actor with router-like behaviour
might do the job just as well as a full-blown router. As explained
